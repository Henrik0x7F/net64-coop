...
private:
    ...
    struct DeriveFilter : IMessageHandler<Identifier>
    {
        struct Handler
        {
            Derived& obj_;
        ...
        {
            custom_dispatch<Handler, MessageIdPredicate, const MessageType&, Messages...>(
                Handler{static_cast<Derived&>(*this)}, msg);
        }
    };

public:
    /**
     * Base for message handlers
     * Forwards all message types to Derived::on_message(const MessageType&)
     */
    template<typename Derived>
    struct Derive : IMessageHandler<Identifier>
    {
        /**
         * Base for message handlers
        ...
        void handle_message(const MessageType& msg) final { static_cast<Derived&>(*this).on_message(msg); }
    };
